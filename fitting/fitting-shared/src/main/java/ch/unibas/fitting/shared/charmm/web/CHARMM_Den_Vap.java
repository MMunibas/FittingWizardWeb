...
public class CHARMM_Den_Vap {
    ...
    public CHARMM_Den_Vap(Settings settings)
    {
        ...
        this.scriptFile = new File(
                settings.getScriptsDir(),
                "submit-remote-charmm.py");
    }

    public CHARMM_Output_GasPhase execute(
            CHARMM_Input_GasPhase input,
            ClusterParameter clusterParameter) {
        LOGGER.debug("execute CHARMM_Input_GasPhase");
        File output = execute("gas_phase.out", input, clusterParameter);
        return new CHARMM_Output_GasPhase(output);
    }

    public CHARMM_Output_PureLiquid execute(
            CHARMM_Input_PureLiquid input,
            ClusterParameter clusterParameter) {
        LOGGER.debug("execute CHARMM_Input_PureLiquid");
        File output = execute("pure_liquid.out", input, clusterParameter);
    ...
    public File execute(
        ...
        List<String> args = prepare_Python(input, clusterParameter, charmmout);
        runner.exec(scriptFile, args);

        return charmmout;
    }

    private List<String> prepare_Python(
            CHARMM_Input input,
            ClusterParameter clusterParameter,
            File output) {
