...
module characters
  ...
  character(len=250) :: cubefile, vdwfile, punfile, basename, punheader
...
! Read and discard lines until line with 'Rank' is encountered, determine type of punfile,
! then go back one line

do while (n2 == 0)
  read(28,'(A)') punheader
  n2 = index(punheader, 'Rank')
  if (n2 /= 0) then
    read(punheader,*,iostat=Error) wrd, wrd, punprobe
    backspace(unit=28)
  endif
end do
...
! read routine for nonlinear molecules
...
! read routine for linear molecules   

else
  do n1=1, natoms
    read(28,*) wrd, zs(n1), rnk, irank(n1)
    xs(n1) = 0
    ys(n1) = 0
    zs(n1) = zs(n1) * a2b
    if (irank(n1).ne.jrank(n1)) then
