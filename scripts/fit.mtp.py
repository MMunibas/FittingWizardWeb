...
offshoot_threshold = 0.1        # Finish increasing restraint #2 if all monopoles are within this threshold to the PC-only fit
...
def read_coeffs(list_files):
    ...
    for i,infile in enumerate(list_files):
        ...
        f.close()
        col_idx = []
        for j in xrange(1,len(readf[0])-1):
...
def add_penalties(chg_control=True):
    ...
    for i in ncoeff_entries.iterkeys():
