...
lmax = 5 #maximum l (must not be higher than 5)
qtot = 0 #total charge
...
for i in range(len(sys.argv)):
  if sys.argv[i] == '-pot':
    esp_cube = GaussianCube(sys.argv[i+1])
  elif sys.argv[i] == '-dens':
    dens_cube = GaussianCube(sys.argv[i+1])
  elif sys.argv[i] == '-lmax':
    lmax = int(sys.argv[i+1])
  elif sys.argv[i] == '-qtot':
    qtot = int(sys.argv[i+1])
  elif sys.argv[i] == '-h':
    usage()
    exit(0)

if esp_cube == '':
  print ("Could not recognize ESP cube file basename.")
  usage()
  exit(0)
if dens_cube == '':
  print ("Could not recognize density cube file basename.")
  usage()
  exit(0)

#read cube files
#dens_cube = GaussianCube("dens_avqz_symmetric_coarse.cube")
#esp_cube = GaussianCube("esp_avqz_symmetric_coarse.cube")

#extract grid values
dens_xyz, dens_val = dens_cube.to_grid_vals()
esp_xyz, esp_val = esp_cube.to_grid_vals()

#atom coordinates
atoms_xyz = np.asarray(esp_cube.atomsXYZ)
...
print("Qtot", np.sum(x[:7]))
print("# RMSE: ", rmse, "Qa(l,m) [a = atom index]")
print(lmax)
for l in range(lmax+1):
    for a in range(Natom):
        for m in range(-l,l+1):
            print('Q'+str(a+1)+'('+str(l)+','+str(m)+')', '     ', x[i])
            i += 1
