...
#    no_sym = False
    ...
    for atom in mol.atoms:
      ...
      TM = Get_local_XYZ(atom.coords,atom.refkind,RC)    # TM is the local XYZ written in the coordinates of the global XYZ  (MD: changed syntax from mtp_tools.Get_local_XYZ)
    ...
    tot_chrg = int(np.round(tot_chrg))  # MD: was tot_chrg = int(round(tot_chrg))
    ...
    if bondincr == True and tot_chrg != 0:
      ...
      for j in range(len(atypes)*9):
        if j%9 == 0: 
          line = line + str(atype_count[int(j/9)]*prefactor) + ' '
        elif j not in excl_from_tab:
          line = line + '0.0 '
      line = line + str(tot_chrg*prefactor) + '\n'
      f.write(line)
