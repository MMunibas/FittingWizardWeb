...
def read_coeffs(list_files,row, col, dat, esp, esp_chg, row_chg, col_chg, dat_chg,rows_per_conf, ncoeff_entries, row_i, chrg_i, coeffs_names, eps, tabf, penalty_swt):
...
#    esp_chg = np.zeros(0,dtype=np.float)
    ...
    return ncoeff_entries,row_i,chrg_i,row,col,dat,rows_per_conf,row_chg,col_chg,dat_chg,esp,esp_chg


def calc_residuals(tabf,Amat,Bvec,coeffs,perconf=False):
...
def add_penalties(coeffs_names,row_chg,col_chg,dat_chg,esp_chg,row_i,chrg_i,ncoeff_entries,x_comb_init,pref_cc,prefactor,npenalties,row,col,dat,esp,chg_control=True):
    ...
    for i in ncoeff_entries.keys():
        if ncoeff_entries[i] > 0:
            ...
            npenalties += 1
    return npenalties,row,col,dat,esp


def del_penalties(row_chg,col_chg,dat_chg,chrg_i,npenalties,row,col,dat,esp,chg_control=True):
...
def measure_chg_control(dat_chg,row_chg,col_chg,x_i,esp_chg):
...
def optimization(penalize,row, col, dat, esp, esp_chg, row_chg, col_chg, dat_chg,rows_per_conf, ncoeff_entries,coeffs_names,x_comb_init,rmse,x_i,prefactor,pref_cc_ini,row_i,chrg_i,tabf,penalty_swt,pref_ini,outpre,verb,num_bi,perconf,offshoot_threshold):
...
# Build reference matrix to calculate residuals.
    print("Creating sparse matrix with shape %i x %i\n" %(row_i,len(coeffs_names)))
...
# 1st: find appropriate pref_cc to control total charge
    ...
    npenalties,row,col,dat,esp=add_penalties(coeffs_names,row_chg,col_chg,dat_chg,esp_chg,row_i,chrg_i,ncoeff_entries,x_comb_init,pref_cc,prefactor,npenalties,row,col,dat,esp,False)
    A = sps.coo_matrix((dat,(row,col)),shape=(row_i+npenalties,len(coeffs_names))).tocsr()
    b = np.array(esp)
    x_i = iterative(A, b)
    npenalties,row,col,dat,esp=del_penalties(row_chg,col_chg,dat_chg,chrg_i,npenalties,row,col,dat,esp,False)
    max_chg_dev = measure_chg_control(dat_chg,row_chg,col_chg,x_i,esp_chg)
    residuals = calc_residuals(tabf,A_ref,b_ref,x_i)
...
# Next rounds with gradual increase of charge control until max charge deviation is less than 1e-12
    ...
    while max_chg_dev > 1e-10 and pref_cc < 1e+12:                        
        print ("MTP fitting interation %4i" %niter)
        pref_cc *= 10
        ...
        b = np.array(esp)
        x_i = iterative(A, b)
        npenalties,row,col,dat,esp=del_penalties(row_chg,col_chg,dat_chg,chrg_i,npenalties,row,col,dat,esp)
        new_max_chg_dev = measure_chg_control(dat_chg,row_chg,col_chg,x_i,esp_chg)
        residuals = calc_residuals(tabf,A_ref,b_ref,x_i)
...
# calculate largest charge coefficients and residuals
        ...
        residuals = calc_residuals(tabf,A_ref,b_ref,x_i,perconf=perconf)
...
# Calculate new coefficients with adapted threshold
    ...
    calc_residuals(tabf,A_ref,b_ref,x_i,perconf=perconf)
    rmse = residuals[2]
    return rmse,x_i
...
def opt_esp(list_files,penalize,rnk,ncoeff_entries,coeffs_names_all,coeffs_names,x_i,rmse,prefactor,pref_cc_ini,x_comb_init,off_hyd,eps,penalty_swt,pref_ini,outpre,verb,num_bi,perconf,offshoot_threshold):
...
# Get coefficient names that should be fit
    ...
    row_i=0
    chrg_i=0
    ncoeff_entries,row_i,chrg_i,row,col,dat,rows_per_conf,row_chg,col_chg,dat_chg,esp,esp_chg=read_coeffs(list_files,row, col, dat, esp, esp_chg, row_chg, col_chg, dat_chg,rows_per_conf, ncoeff_entries, row_i, chrg_i, coeffs_names, eps, list_files, penalty_swt)
    try:
        x_comb_init
    except:
    ...
    rmse,x_i=optimization(penalize,row, col, dat, esp, esp_chg, row_chg, col_chg, dat_chg,rows_per_conf, ncoeff_entries,coeffs_names,x_comb_init,rmse,x_i,prefactor,pref_cc_ini,row_i,chrg_i,list_files,penalty_swt,pref_ini,outpre,verb,num_bi,perconf,offshoot_threshold)
    if penalize:
...
def fit_mtp(rank, charges, outfile, penalty, off_hyd, tabfile): 
  try:
    ...
    coeffs_names_all = []
    coeffs_names_mol = []
    x_i = np.zeros(0,dtype=np.float)
    rmse = None
 
    #######################
    # Set variable defaults
   
    print ("Starting fit_mtp with parameters: rank=%i outfile=%s penalty=%f off_hyd=%r tabfile=%s\n" %(rank, outfile, penalty, off_hyd, tabfile))
    verb = False    
...
#            tabf.append(sys.argv[i])
    ...
    print ()
    coeffs_names=[]
    rmse,x_i=opt_esp(tabf,True,rank,ncoeff_entries,coeffs_names_all,coeffs_names,x_i,rmse,prefactor,pref_cc_ini,x_comb_init,off_hyd,eps,penalty_swt,pref_ini,outpre,verb,num_bi,perconf,offshoot_threshold)
